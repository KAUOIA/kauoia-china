...
[한국어](#한국항공대학교-국제교류처-중문-홈페이지) | **中文**

面向中国留学生的国际交流处介绍网站。不使用框架和服务器，只用 HTML·CSS·JavaScript 的静态网站，通过 GitHub Pages 发布。

- 原始仓库：<https://github.com/KAUOIA/kauoia-china>
- 网站地址：<https://kauoia.github.io/kauoia-china/>
...
```
index.html              页面上的全部文字和结构
README.md               本文档

notices/                ★ 通知公告 —— 发布新通知时只看这个文件夹即可
  notices.js            通知列表（标题·日期·文件名）
  images/               文章图片（点击后放大显示的长图）
  thumbs/               列表用缩略图（文章里的一张照片）

documents/              ★ 下载用 PDF —— 每学期更换（招生简章、奖学金制度等）

assets/                 网站本身 —— 只有修改设计·功能时才动
  css/
    styles.css          全站样式
    gallery.css         通知公告画廊样式
  js/
    main.js             菜单、滚动、交通指南弹窗
    gallery.js          通知公告画廊动作
  images/               校徽、首页大图、介绍·生活服务图片
  vendor/fontawesome/   图标（Font Awesome 6.4.0，不依赖外部 CDN，放在本站内）

tools/
  check-site.mjs        发布前自动检查脚本
.github/workflows/
  static.yml            推送到 main 后先检查再自动发布
```
...
| 想做的事 | 要改的地方 |
| --- | --- |
| 添加·删除通知公告 | `notices/`（见下面第 2 节） |
| 更换招生简章等 PDF | `documents/` + `index.html` 里的下载链接（见下面第 3 节） |
| 修改页面上的说明文字 | `index.html` |
| 颜色·间距·字号等设计 | `assets/css/styles.css` |
| 更换介绍·生活服务图片 | `assets/images/`（用相同文件名覆盖） |
| 按钮·弹窗等动作 | `assets/js/` |
...
| 文件 | 上传位置 | 说明 |
| --- | --- | --- |
| 文章图片 | `notices/images/` | 例：`inform20.webp` |
| 缩略图 | `notices/thumbs/` | 例：`inform20.webp` —— **从文章里挑一张照片即可**。比例不限（会自动裁切），建议宽度 600px 以内 |

> 📦 **请把图片转换成 WebP 再上传。** 文字清晰度不变，文件大小约减少到 1/8。
> 上传 PNG·JPG 也能正常显示，但在中国大陆访问时加载会变慢。
> 可以用 <https://squoosh.app> 这类网页工具，按 `WebP / Quality 80` 左右保存。
> （如果改了文件扩展名，下面的 `notices.js` 里也要改成一样的）

> 准备缩略图有困难时可以省略。此时会显示文章图片的上半部分（标题部分）。

**② 在 `notices/notices.js` 最前面添加一项**（越靠上越新）

```js
export const notices = [
    {
        src: "inform20.webp",      // 文章图片文件名（不用写文件夹，只写文件名）
        thumb: "inform20.webp",    // 缩略图文件名（没有就删掉这一行）
        title: "通知标题（中文）",
        date: "2026-09-20",        // YYYY-MM-DD
        tag: "校内新闻",            // 校内新闻 或 信息公告
        tagClass: "tag-blue"       // 校内新闻用 tag-blue，信息公告用 tag-gold
    },
    {
        src: "inform19.webp",      // ← 已有的项目
        ...
```

⚠️ 最常见的错误：**项目之间漏写逗号（`},`）**、把引号写成中文或韩文引号（`“ ”`）。
即使写错，也会被发布前的自动检查拦下，**不会把出错的版本发布到网站上**。

**③ 提交后自动发布** —— 在 Actions 标签页看到绿色对勾就完成了。

---

## 3. 更换 PDF

1. 把新的 PDF 上传到 `documents/`。
2. 在 `index.html` 里搜索原来的文件名，改成 `href="documents/新文件名.pdf"`。
3. 删除不再使用的 PDF。

> ⚠️ 更改 PDF 文件名会**改变它的网址**。如果曾在微信·小红书等处直接分享过该 PDF 的链接，那些链接会失效，请注意。

---

## 4. 待改进事项

- [ ] 响应式细节调整（在实际设备上确认，微调字号和间距）
- [ ] 在中国大陆实地确认访问情况（页面加载速度、地图按钮是否可用）
